...
def _collections_binding(semantic_id: str) -> dict:
    raw = (
        resources.files("keel_verifier")
        .joinpath("data/permit_to_x/semantic_registry/v12.json")
        .read_bytes()
    )
    registry = json.loads(raw)
    entry = next(
    ...
    presentation = json.loads(
        resources.files("keel_verifier")
        .joinpath("data/permit_to_x/presentation_registry/v11.json")
        .read_text(encoding="utf-8")
    )
    profile = next(
        item for item in presentation["profiles"] if item["semantic_id"] == semantic_id
...
def _insurance_claims_binding(semantic_id: str) -> dict:
    raw = (
        resources.files("keel_verifier")
        .joinpath("data/permit_to_x/semantic_registry/v13.json")
        .read_bytes()
    )
    registry = json.loads(raw)
    entry = next(
    ...
    presentation = json.loads(
        resources.files("keel_verifier")
        .joinpath("data/permit_to_x/presentation_registry/v12.json")
        .read_text(encoding="utf-8")
    )
    profile = next(
        item for item in presentation["profiles"] if item["semantic_id"] == semantic_id
...
def _erp_crm_binding(semantic_id: str) -> dict:
    raw = (
        resources.files("keel_verifier")
        .joinpath("data/permit_to_x/semantic_registry/v14.json")
        .read_bytes()
    )
    registry = json.loads(raw)
    entry = next(
    ...
    presentation = json.loads(
        resources.files("keel_verifier")
        .joinpath("data/permit_to_x/presentation_registry/v13.json")
        .read_text(encoding="utf-8")
    )
    profile = next(
        item for item in presentation["profiles"] if item["semantic_id"] == semantic_id
...
def _procurement_ap_binding(semantic_id: str) -> dict:
    raw = (
        resources.files("keel_verifier")
        .joinpath("data/permit_to_x/semantic_registry/v15.json")
        .read_bytes()
    )
    registry = json.loads(raw)
    entry = next(
    ...
    presentation = json.loads(
        resources.files("keel_verifier")
        .joinpath("data/permit_to_x/presentation_registry/v14.json")
        .read_text(encoding="utf-8")
    )
    profile = next(
        item for item in presentation["profiles"] if item["semantic_id"] == semantic_id
...
@pytest.mark.parametrize(
    ...
    (
        (
            "keel.action.collections_payment_collect.v1",
            "AI Permit-to-Collect-Payment",
            {"obligation_reference_commitment": {"digest": "sha256:" + "a" * 64}},
            "obligation commitment sha256:" + "a" * 64,
        ),
        (
            "keel.action.collections_payment_plan_create.v1",
            "AI Permit-to-Create-Payment-Plan",
            {"obligation_reference_commitment": {"digest": "sha256:" + "b" * 64}},
            "obligation commitment sha256:" + "b" * 64,
        ),
        (
            "keel.action.collections_autopay_change.v1",
            "AI Permit-to-Change-Autopay",
            {"plan_reference_commitment": {"digest": "sha256:" + "c" * 64}},
            "payment plan commitment sha256:" + "c" * 64,
        ),
        (
            "keel.action.collections_notice_send.v1",
            "AI Permit-to-Send-Collections-Notice",
            {
            ...
            "obligation commitment sha256:" + "d" * 64,
        ),
    ),
)
def test_human_artifact_resolves_collections_title_target_and_lifecycle(
    semantic_id: str,
    expected_title: str,
    facts: dict,
    expected_target: str,
) -> None:
    binding = _collections_binding(semantic_id)
    report = _report(
        [_claim("permit.decision.v1", "supported")],
        artifact={
            "kind": "permit_exact",
            ...
            "permit": {
                "profile": "keel.permit_exact/v3",
                "permit_id": "permit-collections",
                "project_id": "project-1",
                "decision": "allow",
                "agent": "collections-arrangement-agent",
                "authorized_action": binding["action_name"],
                "issued_at": "2026-08-10T12:00:00Z",
                "expires_at": "2026-08-10T12:05:00Z",
                "semantic_id": semantic_id,
...
@pytest.mark.parametrize(
    ("semantic_id", "expected_title"),
    (
        (
            "keel.action.insurance_claim_decision_record.v1",
            "AI Permit-to-Decide-Claim",
        ),
        (
            "keel.action.insurance_claim_settlement_set.v1",
            "AI Permit-to-Settle-Claim",
        ),
        (
            "keel.action.insurance_claim_payment_send.v1",
            "AI Permit-to-Pay-Claim",
        ),
        (
            "keel.action.insurance_claim_notice_send.v1",
            "AI Permit-to-Send-Claim-Determination-Notice",
        ),
    ),
)
def test_human_artifact_resolves_insurance_claims_title_target_and_lifecycle(
    semantic_id: str,
    expected_title: str,
) -> None:
    binding = _insurance_claims_binding(semantic_id)
    ...
    report = _report(
        ...
        artifact={
            ...
            "permit": {
                "profile": "keel.permit_exact/v3",
                "permit_id": "permit-insurance-claims",
                "project_id": "project-1",
                "decision": "allow",
                ...
                "semantic_binding": binding,
                "authorization_facts": {
                    "claim_reference_commitment": {"digest": claim_digest}
                },
            },
        },
    )
    ...
    assert human is not None
    assert human["title"] == expected_title
    assert human["authorization"]["target"] == (
        f"insurance claim commitment {claim_digest}"
    )
...
@pytest.mark.parametrize(
    ...
    (
        (
            "keel.action.crm_deal_stage_change.v1",
            "AI Permit-to-Change-Deal-Stage",
            {"deal_reference_commitment": {"digest": "sha256:" + "d" * 64}},
            "synthetic deal commitment sha256:" + "d" * 64,
        ),
        (
            "keel.action.crm_customer_record_update.v1",
            "AI Permit-to-Update-Customer-Record",
            {
                "customer_record_reference_commitment": {
                    "digest": "sha256:" + "e" * 64
                }
            },
            "synthetic customer record commitment sha256:" + "e" * 64,
        ),
        (
            "keel.action.crm_quote_create.v1",
            "AI Permit-to-Create-Quote",
            {"quote_title_commitment": {"digest": "sha256:" + "f" * 64}},
            "draft quote commitment sha256:" + "f" * 64,
        ),
    ),
)
def test_human_artifact_resolves_erp_crm_title_target_and_lifecycle(
    semantic_id: str,
    expected_title: str,
    facts: dict,
    expected_target: str,
) -> None:
    binding = _erp_crm_binding(semantic_id)
    report = _report(
        [_claim("permit.decision.v1", "supported")],
        artifact={
            "kind": "permit_exact",
            ...
            "permit": {
                "profile": "keel.permit_exact/v3",
                "permit_id": "permit-erp-crm",
                "project_id": "project-1",
                "decision": "allow",
                "agent": "erp-crm-operations-agent",
                "authorized_action": binding["action_name"],
    ...
    assert human["authorization"]["target"] == expected_target
    assert expected_target in human["summary"]["text"]
    assert human["lifecycle"]["issued_at"] == "2026-08-11T01:00:00Z"
    assert human["lifecycle"]["expires_at"] == "2026-08-11T01:05:00Z"
...
@pytest.mark.parametrize(
    ...
    (
        (
            "keel.action.procurement_vendor_create.v1",
            "AI Permit-to-Create-Vendor",
            {"vendor_name_commitment": {"digest": "sha256:" + "1" * 64}},
            "synthetic vendor commitment sha256:" + "1" * 64,
        ),
        (
            "keel.action.procurement_purchase_order_issue.v1",
            "AI Permit-to-Issue-Purchase-Order",
            {
            ...
            "synthetic purchase order commitment sha256:" + "2" * 64,
        ),
        (
            "keel.action.procurement_spend_commit.v1",
            "AI Permit-to-Commit-Procurement-Spend",
            {
                "purchase_order_reference_commitment": {
                    "digest": "sha256:" + "4" * 64
                }
            },
            "synthetic purchase order commitment sha256:" + "4" * 64,
        ),
        (
            "keel.action.ap_invoice_approve.v1",
            "AI Permit-to-Approve-Invoice",
            {
            ...
            "synthetic invoice commitment sha256:" + "5" * 64,
        ),
        (
            "keel.action.ap_invoice_duplicate_reject.v1",
            "AI Permit-to-Reject-Duplicate-Invoice",
            {"invoice_reference_commitment": {"digest": "sha256:" + "7" * 64}},
            "synthetic invoice commitment sha256:" + "7" * 64,
        ),
        (
            "keel.action.ap_invoice_payment_release.v1",
            "AI Permit-to-Release-Invoice-Payment",
            {
                "invoice_reference_commitment": {
                    "digest": "sha256:" + "8" * 64
                },
            ...
            "synthetic invoice commitment sha256:" + "8" * 64,
        ),
    ),
)
def test_human_artifact_resolves_procurement_ap_title_target_and_lifecycle(
    semantic_id: str,
    expected_title: str,
    facts: dict,
    expected_target: str,
) -> None:
    binding = _procurement_ap_binding(semantic_id)
    report = _report(
        [_claim("permit.decision.v1", "supported")],
        artifact={
            "kind": "permit_exact",
            ...
            "permit": {
                "profile": "keel.permit_exact/v3",
                "permit_id": "permit-procurement-ap",
                "project_id": "project-1",
                "decision": "allow",
                "agent": "procurement-ap-agent",
                "authorized_action": binding["action_name"],
