...
def test_unpublished_candidate_never_falls_back_to_prior_release(
    ...
    assert manifest["version_tag"] == "v3.27.0"
    assert all(
        "v3.26.0" not in manifest[field]
        for field in (
            "expected_signing_identity",
    ...
    def unavailable(url: str, **kwargs) -> bytes:
        ...
        raise self_check.SelfCheckError(
            ...
            "3.27.0 signed release artifacts do not exist",
        )
    ...
    assert "/v3.27.0/" in requested_urls[0]
    assert "/v3.26.0/" not in requested_urls[0]
